...
const (
	...
	statePrompt
	stateResult
	stateInputExtra state = iota + 100 // valor alto para nÃ£o conflitar
	stateSuggest
	statePagination
)
...
type model struct {
	...
	suggestions    []string // sugestÃµes para navegaÃ§Ã£o
	suggestIndex   int      // Ã­ndice selecionado na lista de sugestÃµes
	// Pagination fields
	currentPage    int    // pÃ¡gina atual
...
// Subcomandos para autocomplete dinÃ¢mico
		...
		case tea.KeyMsg:
			...
			case "enter":
				...
				if m.state == stateDomainSelect {
					m.input.SetValue("")
					return m, nil // nÃ£o vai para stateResult
				}
				m.result = res
				// processCommand jÃ¡ define o estado corretamente (statePagination ou stateResult)
				// entÃ£o nÃ£o precisamos sobrescrever aqui
			...
			case "tab":
				...
				if strings.HasPrefix(current, "list ") {
					...
					pattern := prefix
					if pattern == "" {
						pattern = " " // forÃ§a fuzzy sugerir todos
					}
					...
					// Se nÃ£o houver match, nÃ£o faz nada
...
func (m model) View() string {
	...
	case stateSuggest:
		...
		sb.WriteString("\n(TAB/â†“/j for down, â†‘/k for up, ENTER to select, ESC to cancel)")
		return sb.String()
	case statePagination:
		paginationInfo := fmt.Sprintf("\nPage %d of %d", m.currentPage, m.totalPages)
		navInfo := ""
		if m.totalPages > 1 {
			navInfo = "\n(â†/h: previous page, â†’/l: next page, ENTER/ESC/q: back to prompt)"
		} else {
...
func (m *model) exportCommand(cmd string) string {
	...
	if len(args) < 2 {
		return "Usage: export files|lhf|credentials|all [--format json|csv|html] [to <file>]"
	}
	tipo := args[1]
	format := "json"
	outFile := ""
	...
	if format != "json" && format != "csv" && format != "html" {
		return fmt.Sprintf("Unknown format %q. Use: json, csv, html.", format)
	}
	if outFile == "" {
		outFile = getExportFilename(tipo, format)
	}
	var err error
	switch tipo {
	case "files":
		err = m.exportFiles(outFile, format)
	case "lhf":
		err = m.exportLHF(outFile, format)
	case "credentials":
		err = m.exportCredentials(outFile, format)
	case "all":
...
func (m *model) exportFiles(outFile, format string) error {
	...
	switch format {
	case "csv":
		headers := []string{"ID", "Path", "Host", "Share", "Domain", "User", "Size", "ModTime", "FileType", "MatchPattern", "MatchType", "LocalPath", "FoundTime"}
		rows := make([][]string, len(files))
		for i, r := range files {
			rows[i] = []string{fmt.Sprint(r.ID), r.Path, r.Host, r.Share, r.Domain, r.User, fmt.Sprint(r.Size), r.ModTime, r.FileType, r.MatchPattern, r.MatchType, r.LocalPath, r.FoundTime}
		}
		return writeCSV(f, headers, rows)
	case "html":
		headers := []string{"ID", "Path", "Host", "Share", "Domain", "User", "Size", "ModTime", "FileType", "MatchPattern", "MatchType", "LocalPath", "FoundTime"}
		rows := make([][]string, len(files))
		for i, r := range files {
			rows[i] = []string{fmt.Sprint(r.ID), r.Path, r.Host, r.Share, r.Domain, r.User, r.SizeFormatted, r.ModTime, r.FileType, r.MatchPattern, r.MatchType, r.LocalPath, r.FoundTime}
		}
		return writeHTML(f, "Exported Files", headers, rows)
	default:
...
func (m *model) exportLHF(outFile, format string) error {
	...
	switch format {
	case "csv":
		headers := []string{"ID", "Path", "Host", "Share", "Domain", "User", "Size", "MatchPattern", "MatchType", "ScanMode"}
		rows := make([][]string, len(lhf))
		for i, r := range lhf {
			rows[i] = []string{fmt.Sprint(r.ID), r.Path, r.Host, r.Share, r.Domain, r.User, r.SizeFormatted, r.MatchPattern, r.MatchType, r.ScanMode}
		}
		return writeCSV(f, headers, rows)
	case "html":
		headers := []string{"ID", "Path", "Host", "Share", "Domain", "User", "Size", "MatchPattern", "MatchType", "ScanMode"}
		...
		return writeHTML(f, "Low-Hanging Fruits", headers, rows)
	default:
		enc := json.NewEncoder(f)
		enc.SetIndent("", "  ")
...
func (m *model) exportCredentials(outFile, format string) error {
	...
	switch format {
	case "csv":
		headers := []string{"Domain", "User", "Host", "AuthMethod", "PasswordClear", "PasswordHash", "FoundTime", "IsAdmin"}
		rows := make([][]string, len(creds))
		for i, r := range creds {
			rows[i] = []string{r.Domain, r.User, r.Host, r.AuthMethod, r.PasswordClear, r.PasswordHash, r.FoundTime, fmt.Sprint(r.IsAdmin)}
		}
		return writeCSV(f, headers, rows)
	case "html":
		headers := []string{"Domain", "User", "Host", "AuthMethod", "PasswordClear", "PasswordHash", "FoundTime", "IsAdmin"}
		...
		return writeHTML(f, "Credentials", headers, rows)
	default:
		enc := json.NewEncoder(f)
		enc.SetIndent("", "  ")
...
// writeCSV writes rows to w as RFC 4180 CSV.
...
// writeHTML writes rows to w as a self-contained HTML table report.
func writeHTML(w io.Writer, title string, headers []string, rows [][]string) error {
	fmt.Fprintf(w, `<!DOCTYPE html><html><head><meta charset="utf-8"><title>%s</title>`+
		...
		`table{border-collapse:collapse;width:100%%}th{background:#1e3a5f;color:#7ec8e3;padding:8px 12px;text-align:left}`+
		`td{padding:6px 12px;border-bottom:1px solid #333}tr:hover td{background:#1a1a2e}`+
		`h1{color:#7ec8e3}</style></head><body><h1>%s</h1>`+
		`<p>Generated: %s â€” %d records</p><table><thead><tr>`,
		title, title, time.Now().Format("2006-01-02 15:04:05"), len(rows))
	...
	_, err := fmt.Fprintf(w, "</tbody></table></body></html>")
	return err
}
...
// FunÃ§Ã£o utilitÃ¡ria para parsing de filtros opcionais
...
func getExportFilename(defaultType, format string) string {
	timestamp := time.Now().Format("20060102_150405")
	dir := "exports"
	os.MkdirAll(dir, 0755)
	return filepath.Join(dir, fmt.Sprintf("exported_%s_%s.%s", defaultType, timestamp, format))
}
...
func exportFilesCmd(db *sql.DB, filters ...string) {
	...
	if outFile == "" {
		outFile = getExportFilename("files", "json")
	}
...
func exportLHFCmd(db *sql.DB, filters ...string) {
	...
	if outFile == "" {
		outFile = getExportFilename("lhf", "json")
	}
	if _, err := os.Stat(outFile); err == nil {
		if !confirmOverwrite(outFile) {
			fmt.Println("Export cancelled. File not overwritten.")
...
func exportCredentialsCmd(db *sql.DB, filters ...string) {
	...
	if outFile == "" {
		outFile = getExportFilename("credentials", "json")
	}
	if _, err := os.Stat(outFile); err == nil {
		if !confirmOverwrite(outFile) {
			fmt.Println("Export cancelled. File not overwritten.")
...
func exportAllCmd(db *sql.DB, filters ...string) {
	...
	if outFile == "" {
		outFile = getExportFilename("all", "json")
	}
	if _, err := os.Stat(outFile); err == nil {
		if !confirmOverwrite(outFile) {
			fmt.Println("Export cancelled. File not overwritten.")
...
// FunÃ§Ã£o utilitÃ¡ria para buscar nomes de arquivos do domÃ­nio atual
