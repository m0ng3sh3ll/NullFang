...
import (
	...
	. "github.com/m0ng3sh3ll/NullFang/go-smb2-patch/internal/erref"
	. "github.com/m0ng3sh3ll/NullFang/go-smb2-patch/internal/smb2"

	"github.com/m0ng3sh3ll/NullFang/go-smb2-patch/internal/msrpc"
)
...
func (d *Dialer) DialContext(ctx context.Context, tcpConn net.Conn) (*Session, error) {
	...
	// Empty User is allowed for null/anonymous sessions negotiated at the SMB level.
...
// ShareInfo holds the name and type of an SMB share.
type ShareInfo struct {
	Name string
	Type uint32 // msrpc.ShareType* constants
}

// ListSharesWithTypes lists all shares with their types (disk/printer/IPC/device).
// Uses the same SRVSVC NetShareEnum level-1 call as ListSharenames.
func (c *Session) ListSharesWithTypes() ([]ShareInfo, error) {
	servername := c.addr
	...
	bindReq := &IoctlRequest{
		...
		MaxOutputResponse: 4280,
		Flags:             SMB2_0_IOCTL_IS_FSCTL,
		Input:             &msrpc.Bind{CallId: callId},
	}
	...
	r1 := msrpc.BindAckDecoder(output)
	if r1.IsInvalid() || r1.CallId() != callId {
		return nil, &InvalidResponseError{"broken bind ack response format"}
	}
	...
	if err != nil {
		if rerr, ok := err.(*ResponseError); ok && NtStatus(rerr.Code) == STATUS_BUFFER_OVERFLOW {
			buf := make([]byte, 4280)
			n, rerr2 := f.readAt(buf[:4280-len(output)], 0)
			if rerr2 != nil {
				return nil, rerr2
			}
			output = append(output, buf[:n]...)
	...
	names := r2.ShareNameList()
	types := r2.ShareTypeList()

	infos := make([]ShareInfo, len(names))
	for i, name := range names {
		infos[i].Name = name
		if types != nil && i < len(types) {
			infos[i].Type = types[i]
		}
	}
	return infos, nil
}

// dfsReferralRequest implements Encoder for FSCTL_DFS_GET_REFERRALS input.
type dfsReferralRequest struct {
	encoded []byte // 2-byte MaxReferralLevel + UTF-16LE path + null terminator
}

func newDFSReferralRequest(path string) *dfsReferralRequest {
	b := make([]byte, 2+len(path)*2+2)
	b[0] = 4 // MaxReferralLevel = 4
	b[1] = 0
	for i, r := range path {
		b[2+i*2] = byte(r)
		b[2+i*2+1] = byte(r >> 8)
	}
	return &dfsReferralRequest{encoded: b}
}

func (r *dfsReferralRequest) Size() int    { return len(r.encoded) }
func (r *dfsReferralRequest) Encode(b []byte) { copy(b, r.encoded) }

// DFSGetReferrals issues FSCTL_DFS_GET_REFERRALS on this share to resolve a DFS path.
// Returns the list of target UNC paths. If the server does not support DFS or the path
// is not a DFS junction, an error is returned.
func (fs *Share) DFSGetReferrals(dfsPath string) ([]string, error) {
	f, err := fs.OpenFile(".", os.O_RDONLY, 0666)
	if err != nil {
		return nil, fmt.Errorf("DFS open: %w", err)
	}
	defer f.Close()
	...
	req := &IoctlRequest{
		...
		Flags:             SMB2_0_IOCTL_IS_FSCTL,
		Input:             newDFSReferralRequest(dfsPath),
	}

	output, err := f.ioctl(req)
	if err != nil {
		return nil, fmt.Errorf("DFS ioctl: %w", err)
	}

	return parseDFSReferralResponse(output)
}

// parseDFSReferralResponse parses RESP_GET_DFS_REFERRAL (MS-DFSC 2.2.3).
func parseDFSReferralResponse(data []byte) ([]string, error) {
	if len(data) < 8 {
		return nil, fmt.Errorf("DFS response too short (%d bytes)", len(data))
	}

	numReferrals := int(binary.LittleEndian.Uint16(data[2:4]))
	if numReferrals == 0 {
		return nil, fmt.Errorf("DFS: no referrals in response")
	}

	var targets []string
	offset := 8 // skip PathConsumed(2) + NumberOfReferrals(2) + ReferralHeaderFlags(4)

	for i := 0; i < numReferrals && offset+4 <= len(data); i++ {
		version := int(binary.LittleEndian.Uint16(data[offset : offset+2]))
		...
		case 1:
			if offset+8 <= len(data) {
				netAddrOff = int(binary.LittleEndian.Uint16(data[offset+6 : offset+8]))
			}
		case 2:
			if offset+20 <= len(data) {
				netAddrOff = int(binary.LittleEndian.Uint16(data[offset+18 : offset+20]))
			}
		case 3, 4:
			if offset+18 <= len(data) {
				netAddrOff = int(binary.LittleEndian.Uint16(data[offset+16 : offset+18]))
			}
		}

		if netAddrOff > 0 {
			strStart := offset + netAddrOff
			if target := dfsDecodeUTF16LE(data, strStart); target != "" {
	...
	if len(targets) == 0 {
		return nil, fmt.Errorf("DFS: could not parse any referral targets")
	}
	return targets, nil
}

// dfsDecodeUTF16LE reads a null-terminated UTF-16LE string from data at offset start.
func dfsDecodeUTF16LE(data []byte, start int) string {
	if start >= len(data) {
		return ""
	}
	var runes []rune
