...
// ShareType represents the type of an SMB share (STYPE_* constants from MS-SRVS)
type ShareType = uint32

const (
	ShareTypeDisk   ShareType = 0 // STYPE_DISKTREE
	ShareTypePrint  ShareType = 1 // STYPE_PRINTQ
	ShareTypeDevice ShareType = 2 // STYPE_DEVICE
	ShareTypeIPC    ShareType = 3 // STYPE_IPC
)

// ShareTypeList extracts share types from a level-1 NetShareEnum response.
// Index matches ShareNameList() order.
func (c NetShareEnumAllResponseDecoder) ShareTypeList() []uint32 {
	if len(c) < 48 {
		return nil
	}
	level := le.Uint32(c[24:28])
	...
	count := int(le.Uint32(c[36:40]))
	if len(c) < 48+count*12 {
		return nil
	}
	types := make([]uint32, count)
	for i := 0; i < count; i++ {
		// Fixed SHARE_INFO_1 layout per entry: [name_ptr(4), type(4), comment_ptr(4)]
		types[i] = le.Uint32(c[48+i*12+4 : 48+i*12+8])
	}
	return types
}
