...
import (
	...
	"github.com/m0ng3sh3ll/NullFang/go-smb2-patch/internal/crypto/ccm"
...
func sessionSetup(conn *conn, i Initiator, ctx context.Context) (*session, error) {
	...
	firstStatus := NtStatus(p.Status())
	switch firstStatus {
	case STATUS_SUCCESS, STATUS_MORE_PROCESSING_REQUIRED:
		// ok
	default:
		return nil, &InvalidResponseError{fmt.Sprintf("expected status: %v, got %v", STATUS_MORE_PROCESSING_REQUIRED, firstStatus)}
	}
	...
	// Kerberos non-mutual-auth: Windows returns STATUS_SUCCESS on the first SESSION_SETUP.
	// No second round trip. Set up signing keys using the first-response preauth hash state.
	if firstStatus == STATUS_SUCCESS {
		...
		if s.sessionFlags&(SMB2_SESSION_FLAG_IS_GUEST|SMB2_SESSION_FLAG_IS_NULL) == 0 {
			...
			case SMB311:
				// preauthIntegrityHashValue includes first request+response only.
