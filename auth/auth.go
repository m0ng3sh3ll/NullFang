...
func NewAuthMethod(authType AuthType, params map[string]string) (AuthMethod, error) {
	...
	case AuthTypeKerberos:
		opts := []Option{}
		if ticketPath, ok := params["ticket_path"]; ok && ticketPath != "" {
			opts = append(opts, WithTicketFile(ticketPath))
		}
		if targetHost, ok := params["target_host"]; ok && targetHost != "" {
			opts = append(opts, WithTargetHost(targetHost))
		}
		if spn, ok := params["spn"]; ok && spn != "" {
			opts = append(opts, WithSPN(spn))
		}
		if kdcHost, ok := params["kdc_host"]; ok && kdcHost != "" {
			opts = append(opts, WithKDCHost(kdcHost))
		}
		return NewKerberosAuth(params["domain"], params["username"], opts...), nil
